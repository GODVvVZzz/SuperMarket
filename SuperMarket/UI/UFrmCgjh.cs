...
namespace SuperMarket.UI
{
    public partial class UFrmCgjh : UserControl
    {
        ...
        private void dgBindData()
        {
            ...
            myconn.Open();
            string sql = "select POID as 进货编号, POGSupplier as 供应商, PODate as 进货日期, POGID as 商品编号, POGName as 商品名称, POGCID as 类别, POGUnit as 单位, POGBid as 进价, POGPrice as 售价, POAmount as 进货数量, POGMinimumInventory as 最小库存, POGShelfLife as 保质期, POGProductionDate as 生产日期, POGNotes as 备注, ID as ID from PurchaseOrder";
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                string ssql = "select * from Goods where GID = '" + dataGridView1.Rows[i].Cells[3].Value.ToString().Trim() + "'";
        ...
        private void tsbtn_PODel_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count != 0)
            {
                ...
                if (result == DialogResult.OK)
                {
                    MySqlConnection myconn = BaseClass.DBConn.MyConn();
                    myconn.Open();
                    string sql = "delete from PurchaseOrder where ID = '" + dataGridView1.SelectedCells[14].Value.ToString().Trim() + "'";
                    MySqlCommand mycmd = new MySqlCommand(sql, myconn);
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                MySqlConnection myconn = BaseClass.DBConn.MyConn();
                myconn.Open();
                string sql = "select POID as 进货编号, POGSupplier as 供应商, PODate as 进货日期, POGID as 商品编号, POGName as 商品名称, POGCID as 类别, POGUnit as 单位, POGBid as 进价, POGPrice as 售价, POAmount as 进货数量, POGMinimumInventory as 最小库存, POGShelfLife as 保质期, POGProductionDate as 生产日期, POGNotes as 备注, ID as ID from PurchaseOrder where PODate between '" + begin + "' and '" + end + "'";
                MySqlDataAdapter mysda = new MySqlDataAdapter(sql, myconn);
                DataSet ds = new DataSet();
                ds.Clear();
                mysda.Fill(ds);
