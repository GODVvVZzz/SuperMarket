...
{
    public partial class FrmForget : CCSkinMain
    {
        public FrmForget()
        {
            InitializeComponent();
                        ...
                        else
                        {
                            if (txtbox_NewPasswd.Text == txtbox_CheckNewPasswd.Text)
                            {
                                if (MessageBox.Show("您确定要找回密码吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                                {
                                    ...
                                    myconn.Open();
                                    string sql = "select count(*) from Employee where EID=@username";
                                    ...
                                    if (n == 1)
                                    {
                                        string esql = "select ETel from Employee where ETel='" + txtbox_Tel.Text.Trim() + "'";
                                        DataSet ds = MySqlHelper.ExecuteDataset(myconn, esql);
                                        if (ds.Tables[0].Rows.Count > 0)
                                        {
                                            //修改密码
                                            string csql = "update Employee set EPasswd=@passwd where EID=@username and ETel=@tel";
