...
namespace SuperMarket.UI
{
    public partial class FrmNEGoodsSupplier : Form
    {
        ...
        private void btn_SSubmit_Click(object sender, EventArgs e)
        {
            ...
            myconn.Open();
            string sql = "select count(*) from GoodsSupplier where GSName=@gsname";
            ...
            {
                //修改
                sql = "update GoodsSupplier set GSName=@gsname,GSPName=@gspname,GSTel=@gstel,GSAddress=@gsaddress,GSNotes=@gsnotes where GSID=@gsid";
            ...
            {
                sql = "select count(*) from GoodsSupplier where GSName=@gsname";
                mycmd = new MySqlCommand(sql, myconn);
                ...
                {
                    //添加
                    sql = "insert into GoodsSupplier(GSName,GSPName,GSTel,GSAddress,GSNotes) values(@gsname,@gspname,@gstel,@gsaddress,@gsnotes)";
                    mycmd = new MySqlCommand(sql, myconn);
                    mycmd.Parameters.Add("@gsname", MySqlDbType.VarChar, 20).Value = txtbox_GSName.Text.Trim();
                    mycmd.Parameters.Add("@gspname", MySqlDbType.VarChar, 20).Value = txtbox_GSPName.Text.Trim();
                    mycmd.Parameters.Add("@gstel", MySqlDbType.VarChar, 20).Value = txtbox_GSTel.Text.Trim();
