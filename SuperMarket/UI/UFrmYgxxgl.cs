...
namespace SuperMarket.UI
{
    public partial class UFrmYgxxgl : UserControl
    {
        ...
        string sql;
        public string LN;
       // public SkinSplitContainer skinSplitContainer1 = new SkinSplitContainer();
        ...
        private void cxtj_ayhmcz()
        {
            if (cmbox_cxtj.Text == "按用户名查找")
            {
                ...
                {
                    myconn.Open();
                    sql = "select * from Employee where EID='" + txtbox_cxtjLN.Text + "'";
        ...
        private void cxtj_axmcz()
        {
            if (cmbox_cxtj.Text == "按姓名查找")
            {
                ...
                {
                    myconn.Open();
                    sql = "select * from Employee where EName='" + txtbox_cxtjLN.Text + "'";
                    MySqlCommand mycmd = new MySqlCommand(sql, myconn);
                    int n = Convert.ToInt32(mycmd.ExecuteScalar());
                    if (n > 0)
                    {
        ...
        private void cxtj_azwcz()
        {
            if (cmbox_cxtj.Text == "按职位查找")
            {
                ...
                if (cmbox_cxtjPS.Text == "系统管理员")
                {
                    ...
                    n = 0;
                    myconn.Open();
                    sql = "select EID as 用户名, EName as 姓名, ESex as 性别, EAge as 年龄, EPID as 职位, ETel as 手机, EPasswd as 密码, EAddress as 住址, ID as ID from Employee where EPID='" + p + "'";
        ...
        private void cxtj_axbcz()
        {
            if (cmbox_cxtj.Text == "按性别查找")
            {
                ...
                int n = 0;
                myconn.Open();
                sql = "select EID as 用户名, EName as 姓名, ESex as 性别, EAge as 年龄, EPID as 职位, ETel as 手机, EPasswd as 密码, EAddress as 住址, ID as ID from Employee where ESex='" + cmbox_cxtjPS.Text + "'";
                MySqlDataAdapter mysda = new MySqlDataAdapter(sql, myconn);
                DataSet ds = new DataSet();
                ds.Clear();
                mysda.Fill(ds);
        ...
        private void cxtj_cksyygxx()
        {
            ...
            int n = 0;
            myconn.Open();
            sql = "select EID as 用户名, EName as 姓名, ESex as 性别, EAge as 年龄, EPID as 职位, ETel as 手机, EPasswd as 密码, EAddress as 住址, ID as ID from Employee";
            MySqlDataAdapter mysda = new MySqlDataAdapter(sql, myconn);
            DataSet ds = new DataSet();
            ds.Clear();
            mysda.Fill(ds);
            ...
            {
                dataGridView1.Columns[7].Width = 204;
                string ssql = "select * from Employee where ID='" + dataGridView1.Rows[i].Cells[8].Value.ToString().Trim() + "'";
                MySqlCommand smycmd = new MySqlCommand(ssql, myconn);
                mysdr = smycmd.ExecuteReader();
                mysdr.Read();
                if (mysdr.HasRows)
        ...
        private void cxtj_ckgzwygs()
        {
            ...
            myconn.Open();
            sql = "select EPName as 职位,EPCount as 人数 from EmployeePosition";
            MySqlDataAdapter mysda = new MySqlDataAdapter(sql, myconn);
            DataSet ds = new DataSet();
            ds.Clear();
            mysda.Fill(ds);
        ...
        private void 查看本人信息ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            myconn.Open();
            sql = "select * from Employee where EID='" + LN + "'";
            MySqlCommand mycmd = new MySqlCommand(sql, myconn);
            MySqlDataReader mysdr = mycmd.ExecuteReader();
            mysdr.Read();
            if (mysdr.HasRows)
        ...
        {
            myconn.Open();
            sql = "select sum(EPCount) from EmployeePosition";
            mycmd = new MySqlCommand(sql, myconn);
            int n = Convert.ToInt32(mycmd.ExecuteScalar());
            if (n > 0)
            {
        ...
        {
            myconn.Open();
            sql = "select * from Employee where EID='" + txtbox_LoginName.Text.Trim() + "'";
            mycmd = new MySqlCommand(sql, myconn);
            int n = Convert.ToInt32(mycmd.ExecuteScalar());
            if (n > 0)//修改
            {
                sql = "update Employee set EID=@eid, EName=@ename, ESex=@esex, EAge=@eage, EPID=@epid, ETel=@etel, EPasswd=@epasswd, EAddress=@eaddress where ID=@id";
                mycmd = new MySqlCommand(sql, myconn);
            ...
            {
                sql = "insert into Employee(EID, EName, ESex, EAge, EPID, ETel, EPasswd, EAddress) values (@eid, @ename, @esex, @eage, @epid, @etel, @epasswd, @eaddress)";
                mycmd = new MySqlCommand(sql, myconn);
                mycmd.Parameters.Add("@eid", MySqlDbType.VarChar, 50).Value = txtbox_LoginName.Text.Trim();
                mycmd.Parameters.Add("@ename", MySqlDbType.VarChar, 20).Value = txtbox_Name.Text.Trim();
                mycmd.Parameters.Add("@esex", MySqlDbType.VarChar, 20).Value = cmbox_Sex.Text.Trim();
                ...
                string ssql = "select EPName, EPCount from EmployeePosition where EPName='" + cmbox_Position.Text + "'";
                MySqlCommand smycmd = new MySqlCommand(ssql, myconn);
                ...
                if (smysdr.HasRows)
                {
                    ...
                    smyconn.Open();
                    string sssql = "update EmployeePosition set EPCount=@epcount where EPName=@epname";
        ...
        {
            myconn.Open();
            sql = "delete from Employee where ID='" + Convert.ToInt32(lab_ID.Text.Trim()) + "'";
            mycmd = new MySqlCommand(sql, myconn);
