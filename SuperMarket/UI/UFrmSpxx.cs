...
namespace SuperMarket.UI
{
    public partial class UFrmSpxx : UserControl
    {
        ...
        private void tvBindData()
        {
            ...
            myconn.Open();
            string sql = "select GCName from GoodsCategory";
        ...
        public void dgBindData()
        {
            ...
            MySqlConnection myconn = BaseClass.DBConn.MyConn();
            myconn.Open();
            string sql = "select GID as 商品编号, GName as 商品名称, GCID as 类别, GPrice as 售价, GBid as 进价, GAmount as 库存数量, GMinimumInventory as 最小库存, GUnit as 单位, GShelfLife as 保质期, GProductionDate as 生产日期, GSupplier as 供应商, GNotes as 备注, ID as ID from Goods";
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                string ssql = "select * from Goods where ID = '" + dataGridView1.Rows[i].Cells[12].Value.ToString().Trim() + "'";
                ...
                if (mysdr.HasRows)
                {
                    ...
                    mysdr.Dispose();
                    string gcsql = "select GCID, GCName from GoodsCategory where GCID='" + gcid + "'";
        ...
        private void tsbtn_Del_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count != 0)
            {
                ...
                if (result == DialogResult.OK)
                {
                    MySqlConnection myconn = BaseClass.DBConn.MyConn();
                    myconn.Open();
                    string sql = "delete from Goods where ID = '" + dataGridView1.SelectedCells[12].Value.ToString().Trim() + "'";
                    MySqlCommand mycmd = new MySqlCommand(sql, myconn);
        ...
        private void tsbtn_Search_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection myconn = BaseClass.DBConn.MyConn();
            myconn.Open();
            string sql = "select GID as 商品编号, GName as 商品名称, GCID as 类别, GPrice as 售价, GBid as 进价, GAmount as 库存数量, GMinimumInventory as 最小库存, GUnit as 单位, GShelfLife as 保质期, GProductionDate as 生产日期, GSupplier as 供应商, GNotes as 备注, ID as ID from Goods where GID = '" + tstxtbox_spbhmc.Text.Trim() + "' or GName = '" + tstxtbox_spbhmc.Text.Trim() + "'";
            MySqlCommand mycmd = new MySqlCommand(sql, myconn);
            MySqlDataReader mysdr = mycmd.ExecuteReader();
            mysdr.Read();
            if (mysdr.HasRows)
        ...
        private void treeView1_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            else
            {
                MySqlConnection myconn = BaseClass.DBConn.MyConn();
                myconn.Open();
                string tvmysql = "select * from GoodsCategory where GCName='" + e.Node.Text + "'";
                ...
                if (tvmysdr.HasRows)
                {
                    int n = 0;
                    dataGridView1.Columns.Clear();
                    string sql = "select GID as 商品编号, GName as 商品名称, GCID as 类别, GPrice as 售价, GBid as 进价, GAmount as 库存数量, GMinimumInventory as 最小库存, GUnit as 单位, GShelfLife as 保质期, GProductionDate as 生产日期, GSupplier as 供应商, GNotes as 备注, ID as ID from Goods where GCID='" + tvmysdr[0].ToString().Trim() + "'";
                    MySqlDataAdapter mysda = new MySqlDataAdapter(sql, myconn);
                    DataSet ds = new DataSet();
                    tvmysdr.Dispose();
                    ds.Clear();
