...
namespace SuperMarket.UI
{
    ...
    {
        string roid = "RWA" + DateTime.Now.ToString("yyyyMMddhhmmss");//生成退货单号
        public UFrmGkth()
        {
            InitializeComponent();
        }
        ...
        private void UFrmLsjyjl_Load(object sender, EventArgs e)
        {
            ...
            myconn.Open();
            string sql = "select DISTINCT OID from sellorder order by OID desc";
        ...
        private void dgBindData()
        {
            ...
            {
                ordernum = "";
                string allsql = "select OID as 订单编号, OGID as 商品编号, OGName as 商品名称, OGCID as 类别, OGAmount as 数量, OGPrice as 单价, OGUnit as 单位, OGSum as 总价, OPaymentMethod as 付款方式 from sellorder where IsReturn='否' order by OID desc";
                dataGridView1.DataSource = dgBindData(allsql).Tables[0];

                if (dataGridView1.ColumnCount < 10) //大于等于10 则不用重新生成列
                {
            ...
            {
                if (cmbox_OrderNum.SelectedItem != null) //查询某个订单
                {
                    ordernum = cmbox_OrderNum.Text.ToString().Trim();
                    string onsql = "select OID as 订单编号, OGID as 商品编号, OGName as 商品名称, OGCID as 类别, OGAmount as 数量, OGPrice as 单价, OGUnit as 单位, OGSum as 总价, OPaymentMethod as 付款方式 from sellOrder where OID='" + ordernum + "' and IsReturn='否' order by OID desc";
                ...
                if (dataGridView1.Columns[e.ColumnIndex].Name.Equals("提交"))
                {
                    if (dataGridView1.Rows[e.RowIndex].Cells["退货原因"].Value != null)
                    {
                        if (dataGridView1.Rows[e.RowIndex].Cells["退货原因"].Value.ToString() == "1" || dataGridView1.Rows[e.RowIndex].Cells["退货原因"].Value.ToString() == "2" || dataGridView1.Rows[e.RowIndex].Cells["退货原因"].Value.ToString() == "3")
                        {
                            ...
                            MySqlConnection myconn = BaseClass.DBConn.MyConn();
                            myconn.Open();
                            string sql = "INSERT INTO ReturnOrder(OID, ROGName, ROGCID, ROGAomunt, ROGPrice, ROGUnit, ROGSum, ROPaymentMethod, ROReason, ROID) VALUES (@oid, @rogname, @rogcid, @rogaomunt, @rogprice, @rogunit, @rogsum, @ropaymentmethod, @roreason, @roid)";
                            MySqlCommand mycmd = new MySqlCommand(sql, myconn);
                            ...
                            //更新是否退款
                            myconn.Open();
                            sql = "update sellOrder set IsReturn=@isreturn where OID=@oid and OGName=@ogname";
                            ...
                            //增加库存
                            myconn.Open();
                            sql = "select GAmount from Goods where GName=@gname";
                            mycmd = new MySqlCommand(sql, myconn);
