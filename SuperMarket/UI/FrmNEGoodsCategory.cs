    ...
    public partial class FrmNEGoodsCategory : Form
    {
        ...
        private void FrmNRCategory_Load(object sender, EventArgs e)
        {
            ...
            else if (nr == 1)
            {
                ...
                myconn.Open();
                string sql = "select * from GoodsCategory where GCName='" + gcname + "'";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            MySqlConnection myconn = BaseClass.DBConn.MyConn();
            myconn.Open();
            string sql = "select count(*) from GoodsCategory where GCName=@gcname";
            MySqlCommand mycmd = new MySqlCommand(sql, myconn);
            ...
            {
                //修改
                sql = "update GoodsCategory set GCName=@gcname where GCID=@gcid";
            ...
            {
                sql = "select count(*) from GoodsCategory where GCName=@gcname";
                mycmd = new MySqlCommand(sql, myconn);
                ...
                {
                    //添加
                    sql = "insert into GoodsCategory(GCName) values(@gcname)";
                    mycmd = new MySqlCommand(sql, myconn);
                    mycmd.Parameters.Add("@gcname", MySqlDbType.VarChar, 20).Value = txtbox_GCName.Text.Trim();
                    mycmd.ExecuteNonQuery();
                    myconn.Close();
