            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        myconn.Open();
                        string sql = "select count(*) from PurchaseOrder where POID=@poid";
                        ...
                        {
                            //修改
                            sql = "update PurchaseOrder set PODate=@podate,POGID=@pogid,POGName=@pogname,POGCID=@pogcid,POGPrice=@pogprice,POGBid=@pogbid,POAmount=@poamount,POGMinimumInventory=@pogminimuminventory,POGUnit=@pogunit,POGShelfLife=@pogshelflife,POGProductionDate=@pogproductiondate,POGSupplier=@pogsupplier,POGNotes=@pognotes where ID=@id";
                            ...
                            string ssql = "select * from GoodsCategory where GCName='" + cmbox_POGCID.Text + "'";
                            ...
                            if (q > 0)
                            {
                                ...
                                #region 库存数量变化
                                string gssql = "select * from Goods where GName='" + txtbox_POGName.Text + "'";
                                ...
                                if (gsmysdr.HasRows)
                                {
                                    ...
                                    {
                                        gsmysdr.Read();
                                        string gsql = "update Goods set GID=@pogid,GCID=@pogcid,GPrice=@pogprice,GBid=@pogbid,GAmount=@poamount,GMinimumInventory=@pogminimuminventory,GUnit=@pogunit,GShelfLife=@pogshelflife,GProductionDate=@pogproductiondate,GSupplier=@pogsupplier,GNotes=@pognotes where GName=@pogname";
                                        ...
                                        string gcidsql = "select * from GoodsCategory where GCName='" + cmbox_POGCID.Text + "'";
                        ...
                        {
                            sql = "select count(*) from PurchaseOrder where POID=@poid";
                            mycmd = new MySqlCommand(sql, myconn);
                            ...
                            {
                                //添加
                                sql = "insert into PurchaseOrder(POID,PODate,POGID,POGName,POGCID,POGPrice,POGBid,POAmount,POGMinimumInventory,POGUnit,POGShelfLife,POGProductionDate,POGSupplier,POGNotes) values(@poid, @podate, @pogid, @pogname, @pogcid, @pogprice, @pogbid, @poamount, @pogminimuminventory, @pogunit, @pogshelflife, @pogproductiondate, @pogsupplier, @pognotes)";
                                mycmd = new MySqlCommand(sql, myconn);
