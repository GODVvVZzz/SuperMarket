...
namespace SuperMarket.UI
{
    public partial class UFrmDqkc : UserControl
    {
        ...
        public void dgBindData()
        {
            ...
            myconn.Open();
            string sql = "select GID as 商品编号, GName as 商品名称, GCID as 类别, GPrice as 售价, GBid as 进价, GAmount as 库存数量, GMinimumInventory as 最小库存, GSupplier as 供应商, ID as ID from Goods where GAmount <= GMinimumInventory";
            ...
            for (int i = 0; i < n; i++)
            {
                string ssql = "select * from Goods where ID = '" + dataGridView1.Rows[i].Cells[8].Value.ToString().Trim() + "'";
