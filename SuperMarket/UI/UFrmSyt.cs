...
namespace SuperMarket.UI
{
    public partial class UFrmSyt : UserControl
    {
        ...
        private void zljs()
        {
            if (lab_price_yf.Text != "0") //应付不为0
            {
                btn_jzfs_Alipay.Enabled = true; // 支付宝支付按钮可用
                lab_price_zl.Text = "0";
                Regex s = new Regex(@"^[0-9]{1,}([.]{1}[0-9]{1,2})?$");//正则表达式 控制输入为数字
                if (s.IsMatch(txtbox_price_sf.Text.Trim())) 
                {
                    if (Convert.ToDouble(txtbox_price_sf.Text.Trim()) != 0)
                    {
                        if (Convert.ToDouble(txtbox_price_sf.Text.Trim()) - Convert.ToDouble(lab_price_yf.Text.Trim()) >= 0)
                        {
                            ...
                            btn_jzfs_cash.Enabled = true; //应付 足够 现金付款按钮可用
                            lab_warning.Text = "";
                        }
                        else
                        {
        ...
        private void initialize()
        {
            timer1.Start();
            btn_jzfs_cash.Enabled = false;
            lab_OrderNum.Text = "MMS" + DateTime.Now.ToString("yyyyMMddHHmmss"); //根据时间生成订单编号

            //string sql = "select * from tb_Goods where GAmount > 0";
            string sql = "select GID, GName, GCID, GPrice, GUnit, ID from Goods where GAmount > 0";
            ...
            Column1.DisplayMember = "GID"; //设置商品编号下拉选项数据来源
            Column2.DisplayMember = "GName"; //设置商品名下拉选项数据来源
        }
        #endregion
        ...
        private int saveOrder()
        {
            ...
            {
                string sql = "insert into sellOrder (OID, OGID, OGName, OGCID, OGAmount, OGPrice, OGUnit, OGSum, OEID, OPaymentMethod, IsReturn) values (@oid, @ogid, @ogname, @ogcid, @ogamount, @ogprice, @ogunit, @ogsum, @oeid, @opaymentmethod, @isreturn)";
                MySqlCommand mycmd = new MySqlCommand(sql, myconn);
            ...
            for (int i = 0; i < dataGridView1.Rows.Count - 1; i++)
            {
                string sql = "select GAmount from Goods where GName=@gname";
                MySqlCommand mycmd = new MySqlCommand(sql, myconn);
                ...
                if (n > 0)
                {
                    ...
                    if (mysdr.HasRows)
                    {
                        sql = "update Goods set GAmount=@gamount where GName=@gname";
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (x >= 0 && y >= 0 && dataGridView1.Rows[y].Cells[x].Value != null)
            {
                ...
                if (x == 0)
                {
                    ...
                    for (int i = 0; i < dtgoods.Rows.Count; i++)//遍历数据库中所有商品 根据id找到相应商品名
                    {
                        if (dtgoods.Rows[i]["GID"].Equals(dataGridView1.Rows[y].Cells[x].Value))//找到对应的商品编号
                        {
                    ...
                    //查找商品类别
                    string lbsql = "select GoodsCategory.GCName from GoodsCategory,Goods where Goods.GID=@gid and GoodsCategory.GCID=Goods.GCID";
                    ...
                    //查找商品单价
                    string djsql = "select GPrice from Goods where GID=@gid";
                    ...
                    //查找商品单位
                    string dwsql = "select GUnit from Goods where GID=@gid";
                ...
                {
                    string slsql = "select GAmount from Goods where GID=@gid";
        ...
        */
        ...
        {
            Dictionary<int, int> width = GetMaxWidth(dtprintorder); //得到每列最大宽度
            width[0] = 5;  //为了手动解决不知名bug 去掉会使清单打印商品编号后出现大量空白 宽度的问题
            int x = e.MarginBounds.X; //纸张可用范围的x 
            int y = e.MarginBounds.Y;//纸张可用范围的y
            int w = e.MarginBounds.Width + 10;// 打印纸张的宽
            int left;
            int sum = 0;
            ...
            left = (w - sum) / 10 - x;//左侧空白位置
            Graphics g = e.Graphics;
            Brush b = new SolidBrush(Color.Black);//字体颜色
            Font f = new Font("宋体", 18); // 字体格式
            g.DrawString("SuperMarket超市欢迎您", f, b, w / 2, y);// 顶部中间打印
            g.DrawString("您本次消费清单如下：", f, b, x + left, y + 50);//下方靠左侧接着打印
            x = e.MarginBounds.X + left;
            y = e.MarginBounds.Y + 100;// 下移
            for (int k = 0; k < dtprintorder.Columns.Count; k++) //打印列名
            {
                string str = dtprintorder.Columns[k].ColumnName;
                g.DrawString(str, f, b, x, y);
                x = x + width[k] * 30;
            }
            for (int i = 0; i < dtprintorder.Rows.Count; i++)  //打印数据
            {
                x = e.MarginBounds.X + left;
           ...
           // FrmMain fm = new FrmMain();
            ...
            //string s = Convert.ToString(width[0]);
            //g.DrawString(s, f, b, w / 2+200, y + 50 * 10);
        }
        #endregion
