    ...
    public partial class FrmNEGoods : Form
    {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                myconn.Open();
                string sql = "select count(*) from Goods where GID=@gid";
                ...
                {
                    //修改
                    sql = "update Goods set GID=@gid,GName=@gname,GCID=@gcid,GPrice=@gprice,GBid=@gbid,GUnit=@gunit,GSupplier=@gsupplier,GNotes=@gnotes where ID=@ID";
                    ...
                    string ssql = "select * from GoodsCategory where GCName='" + cmbox_GCID.Text + "'";
                ...
                {
                    sql = "select count(*) from Goods where GName=@gname";
                    mycmd = new MySqlCommand(sql, myconn);
                    ...
                    {
                        //添加
                        sql = "insert into Goods(GID,GName,GCID,GPrice,GBid,GAmount,GUnit,GSupplier,GNotes) values(@gid,@gname,@gcid,@gprice,@gbid,@gamount,@gunit,@gsupplier,@gnotes)";
                        mycmd = new MySqlCommand(sql, myconn);
                        mycmd.Parameters.Add("@gid", MySqlDbType.VarChar, 20).Value = txtbox_GID.Text.Trim();
                        mycmd.Parameters.Add("@gname", MySqlDbType.VarChar, 20).Value = txtbox_GName.Text.Trim();
